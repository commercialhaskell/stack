...
* Download [the latest release](https://github.com/commercialhaskell/stack/releases/latest).  Note: the i386 (32-bit) version GHC is better tested on Windows. 
...
**Note**: If you see the message `bash: <PATH_TO_STACK>/stack: Permission denied` or `stack isn't recognized` set the executable bit `chmod +x <PATH_TO_STACK>/stack`
