...
~~~haskell
#!/usr/bin/env stack
-- stack script --snapshot lts-23.8 --package turtle
...
command with the options of a LTS Haskell 23.8 snapshot
(`--snapshot lts-23.8`) and ensuring the
...
snapshot (`lts-23.8` provides `turtle-1.6.2`).
...
~~~text
stack script --snapshot lts-23.8 -- MyScript.hs arg1 arg2 +RTS -s -RTS
~~~
...
command
`stack script --snapshot lts-23.8 --package acme-missiles --no-run --compile -- Script.hs`
