...
~~~haskell
#!/usr/bin/env stack
-- stack script --snapshot lts-24.0 --package turtle
...
command with the options of a LTS Haskell 24.0 snapshot
(`--snapshot lts-24.0`) and ensuring the
...
snapshot (`lts-24.0` provides `turtle-1.6.2`).
...
~~~text
stack script --snapshot lts-24.0 -- MyScript.hs arg1 arg2 +RTS -s -RTS
~~~
...
command
`stack script --snapshot lts-24.0 --package acme-missiles --no-run --compile -- Script.hs`
