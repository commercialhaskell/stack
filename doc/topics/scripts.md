...
~~~haskell
#!/usr/bin/env stack
-- stack script --snapshot lts-24.55 --package turtle
...
command with the options of a LTS Haskell 24.55 snapshot
(`--snapshot lts-24.55`) and ensuring the
...
snapshot (`lts-24.55` provides `turtle-1.6.2`).
...
~~~text
stack script --snapshot lts-24.55 -- MyScript.hs arg1 arg2 +RTS -s -RTS
~~~
...
command
`stack script --snapshot lts-24.55 --package acme-missiles --no-run --compile -- Script.hs`
