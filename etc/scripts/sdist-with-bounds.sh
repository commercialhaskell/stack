...
if [[ -f stack-lts-12.yaml ]]; then
  midyaml=stack-lts-12.yaml
else
  midyaml=stack.yaml
fi
...
rm -f "$middist/stack-$ver.tar.gz"
"$stackbin" --stack-yaml="$midyaml" sdist . --pvp-bounds=both
rm -rf "$middist/_mid" && mkdir "$middist/_mid" && (cd "$middist/_mid" && tar xf "../stack-$ver.tar.gz")
mcabal="$(echo "$middist/_mid/stack-$ver/stack.cabal")"
