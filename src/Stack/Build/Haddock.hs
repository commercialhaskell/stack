...
openHaddocksInBrowser
    ...
    -> Set PackageName
    -- ^ Target packages specified on the CLI
    -> m ()
openHaddocksInBrowser bco pkgLocations targets = do
    inGlobalProject <- asks (bcImplicitGlobal . getBuildConfig)
    docDir <-
        case map (`Map.lookup` pkgLocations) (Set.toList targets) of
            [Just (pkgId, iloc)] -> do
                pkgRelDir <- (parseRelDir . show) pkgId
                let docLocation =
                        case (inGlobalProject, iloc) of
                            (True, Snap) -> snapDocDir bco
                            _ -> localDocDir bco
