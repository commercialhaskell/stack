...
stack is a build tool for Haskell code. It is:
...
Getting started is easy; download the `stack` executable **FIXME add link once
we have a release** and put it on your PATH. You can then run `stack build`
...
* `stack setup` will install whatever version of GHC necessary to build your
  ...
  project requires and compare it to the GHCs available)

* `stack exec` will run a command with your `PATH` and `GHC_PACKAGE_PATH`
  augmented to include your project directories

* `stack ghc` and `stack runghc` can be used to compile and run individual
  files, respectively
