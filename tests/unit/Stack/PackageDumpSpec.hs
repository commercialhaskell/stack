...
spec = do
    ...
    describe "conduitDumpPackage" $ do
        it "ghc 7.8" $ do
            haskell2010:_ <-
                  withSourceFile "tests/unit/package-dump/ghc-7.8.txt" $ \src ->
                  runConduit
                $ src
               .| decodeUtf8
               .| conduitDumpPackage
        ...
        it "ghc 7.10" $ do
            haskell2010:_ <-
                  withSourceFile "tests/unit/package-dump/ghc-7.10.txt" $ \src ->
                  runConduit
                $ src
               .| decodeUtf8
               .| conduitDumpPackage
        ...
        it "ghc 7.8.4 (osx)" $ do
            hmatrix:_ <-
                  withSourceFile "tests/unit/package-dump/ghc-7.8.4-osx.txt" $ \src ->
                  runConduit
                $ src
               .| decodeUtf8
               .| conduitDumpPackage
        ...
        it "ghc HEAD" $ do
          ghcBoot:_ <-
                withSourceFile "tests/unit/package-dump/ghc-head.txt" $ \src ->
                runConduit
              $ src
             .| decodeUtf8
             .| conduitDumpPackage
