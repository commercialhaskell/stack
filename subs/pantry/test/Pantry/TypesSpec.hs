...
|]
       ...
       pkgImm <- case Yaml.decodeThrow lockFile of
         Just (pkgIm :: Value) -> do
           case Yaml.parseEither (parseLockFile rootDir) pkgIm of
             Left str -> fail $ "Can't parse PackageLocationImmutable - 1" <> str
             Right iopl -> do
               pl <- iopl
               pure $ NonEmpty.toList pl
                ...
                Just (pkgIm :: Value) -> do
                    case Yaml.parseEither (parseLockFile rootDir) pkgIm of
                        ...
                        Right iopl -> do
                            ...
                            pure $ NonEmpty.toList pl
                Nothing -> fail "Can't parse PackageLocationImmutable"
        pkgImm `shouldBe` [PLImmutable (PLIHackage (PackageIdentifier {pkgName = mkPackageName "persistent", pkgVersion = mkVersion [2,8,2]}) (toBlobKey
                                 "df118e99f0c46715e932fe82d787fc09689d87898f3a8b13f5954d25af6b46a1" 5058) (TreeKey
                                 (BlobKey
                                      (decodeSHA
                                           "3cb3a9ca3e373a152d9acf622706471578cc93b2ed20c893fc20a4814264f1ce")
